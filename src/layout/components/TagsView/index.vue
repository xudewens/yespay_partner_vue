...
export default {
  ...
  computed: {
    visitedViews() {
      const list = this.$store.state.tagsView.visitedViews.filter(item => item.path !== '/dashboard')
      return list
    },
    routes() {
  ...
  methods: {
    tableDatas() {
      // 是否展示首页标签
      this.menuList = JSON.parse(localStorage.getItem(process.env.VUE_APP_PARAM + '_LIST'))
      try {
