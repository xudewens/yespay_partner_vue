<template>
  <!-- 收款订单 -->
  <div class="dashbody">
    <div class="templateOne" v-if="!dialogTableVisible">
    ...
    <div class="templateTwo" v-else>
      <!-- <el-dialog :title="editOrderType == 0?'补单':'退单'" :visible.sync="dialogTableVisible" width="88%"> -->
            ...
            <el-table-column  label="银行流水ID" width="300">
              <template slot-scope="scope">
                  ...
                  <el-form-item :prop="'submitOrderList.' + scope.$index + '.flowId'" :rules="formData.rules.flowId" style="margin-bottom: 0;">
                    <el-input v-model="scope.row.flowId" :placeholder="'银行流水ID'" v-if="!editOrderType" @change="changecardFlow(scope.$index)"></el-input>
                  </el-form-item>
                  <el-input v-model="scope.row.flowId" disabled  v-if="editOrderType"></el-input>
              </template>
            </el-table-column>
            <el-table-column  label="实收金额（Rp）" v-if="!editOrderType" width="150">
              <template slot-scope="scope">
                <el-input v-model="scope.row.orderPayNum" disabled></el-input>
              </template>
            </el-table-column>
            <el-table-column  label="实收金额（Rp）" v-if="editOrderType" width="150">
              <template slot-scope="scope">
                  <el-input v-if="scope.row.flowId" v-model="scope.row.inMoney" disabled></el-input>
                  <el-input v-else :v-model="0" disabled></el-input>

              </template>
            </el-table-column>
            <el-table-column  label="差额（Rp）" width="150" fixed="right">
              <template slot-scope="scope">
                  <span style="color:red;" v-if="!editOrderType">{{ scope.row.orderPayNum ? scope.row.orderPayNum - (+scope.row.orderMoney + scope.row.floatMoney) : ''}}</span>
                  <span style="color:red;" v-if="editOrderType">{{ scope.row.flowId ? 0 : 0}}</span>    
              </template>
            </el-table-column>
          </el-table>
        </el-form>
        <div class="dialog-footer">
          ...
          <el-button type="primary" v-preventReClick @click="OrderClick">确 定</el-button>
        </div>
      <!-- </el-dialog> -->
    </div>
  </div>
