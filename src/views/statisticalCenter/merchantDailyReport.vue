    ...
    <div class="dashTable">
      <div class="operation">
         ...
         <export-download
          ...
          :is-mark="true"
          file-name="BCA流水列表"
          :expand-fun="mchDailyTrade_export"
          :expand-listfun="mchDayReportExcel"
        />
      </div>
      <el-table
        :data="tableData"
          ...
          <template  slot-scope="scope">
            ...
            >
              ...
              <template slot-scope="scopes">
                  <span v-if="scopes.row.tradeType === 'PAY'">代收订单</span>
                  <span v-if="scopes.row.tradeType === 'LOAN'">代付订单</span>
                  <span v-if="scopes.row.tradeType === 'TOTAL'">代收代付汇总</span>
                </template>
              </el-table-column>
              <el-table-column
                prop="orderTotalNum"
              ...
              >       
                <template slot-scope="scopes">
                  <span>{{ scopes.row.orderTotalNum || 0 | formatNumber}}</span>
                </template>
              </el-table-column>
              <el-table-column
              ...
              >
                <template slot-scope="scopes">
                  <span>{{ scopes.row.orderSuccessNum || 0 | formatNumber}}</span>
                </template>
              </el-table-column>
              <el-table-column
              ...
              >
                <template slot-scope="scopes">
                  <span>{{scopes.row.orderSuccessRate ?`${(scopes.row.orderSuccessRate*100).toFixed(2)}%` : '0%' }}</span>
                </template>
            </el-table-column>
              <el-table-column
                prop="manualOrderNum"
              ...
              >
                <template slot-scope="scopes">
                  <span>{{ scopes.row.manualOrderNum || 0 | formatNumber}}</span>
                </template>
              </el-table-column>
              <el-table-column
                prop="manualOrderRate"
                min-width="80"
                label="补单率"
                align="right"
                show-overflow-tooltip
              >
              <template slot-scope="scopes">
                  <span>{{scopes.row.manualOrderRate ?`${(scopes.row.manualOrderRate*100).toFixed(2)}%` : '0%' }}</span>
              </template>
            </el-table-column>
              <el-table-column
              ...
              >
                <template slot-scope="scopes">
                  <span>{{ scopes.row.orderTotalAmount || 0 | formatNumber}}</span>
                </template>
              </el-table-column>
              <el-table-column
                prop="orderSuccessAmount"
                min-width="130"
                label="订单成功额（Rp）"
                align="right"
                show-overflow-tooltip
              >
               <template slot-scope="scopes">
                  <span>{{ scopes.row.orderSuccessAmount || 0 | formatNumber}}</span>
                </template>
              </el-table-column>
              <el-table-column
                prop="orderServiceAmount"
                min-width="130"
                label="订单手续费（Rp）"
                align="right"
                show-overflow-tooltip
              >
                <template slot-scope="scopes">
                  <span>{{ scopes.row.orderServiceAmount || 0 | formatNumber}}</span>
                </template>
              </el-table-column>
              <el-table-column
                prop="cost"
              ...
              >               
                <template slot-scope="scopes">
                    <span>{{ scopes.row.cost || 0 | formatNumber}}</span>
                </template>
              </el-table-column>
              <el-table-column
                prop="grossProfit"
      ...
      <!-- <el-table-column
        ...
        >
        <template slot-scope="scope">
          <span>{{ timeFormat(scope.row.tradeTime) }}</span>
        </template>
      </el-table-column>
        <!-- 账号 -->
        <el-table-column
        ...
        >
        <template slot-scope="scope">
          <span>{{ scope.row.orderTotalNum || 0 | formatNumber}}</span>
        </template>
      </el-table-column>
        <!-- 订单成功数 -->
        <el-table-column
        ...
        >     
        <template slot-scope="scope">
            <span>{{ scope.row.orderSuccessNum || 0 | formatNumber}}</span>
          </template>
      </el-table-column>
        <!-- 订单成功率 -->
        <el-table-column
        ...
        >
          <template slot-scope="scope">
              <span>{{scope.row.orderSuccessRate ?`${(scope.row.orderSuccessRate*100).toFixed(2)}%` : '0%' }}</span>
          </template>
      </el-table-column>
        <!-- 补单笔数 -->
        <el-table-column
        ...
        >
        <template slot-scope="scope">
            <span>{{ scope.row.manualOrderNum || 0 | formatNumber}}</span>
          </template>
      </el-table-column>
        <!-- 补单率 -->
        <el-table-column
        ...
        >
        <template slot-scope="scope">
            <span>{{scope.row.manualOrderRate ?`${(scope.row.manualOrderRate*100).toFixed(2)}%` : '0%' }}</span>
        </template>
      </el-table-column>
        <!-- 订单总额 -->
        <el-table-column
        ...
        >
         <template slot-scope="scope">
            <span>{{ scope.row.orderTotalAmount || 0 | formatNumber}}</span>
          </template>
        </el-table-column>
        <!-- 订单成功额 -->
        <el-table-column
        ...
        >
         <template slot-scope="scope">
            <span>{{ scope.row.orderSuccessAmount || 0 | formatNumber}}</span>
          </template>
        </el-table-column>
        <!-- 订单手续费 -->
        <el-table-column
        ...
        >
         <template slot-scope="scope">
            <span>{{ scope.row.orderServiceAmount || 0 | formatNumber}}</span>
          </template>
        </el-table-column>
         <!-- 成本 -->
         <el-table-column
        ...
        >
         <template slot-scope="scope">
            <span>{{ scope.row.cost || 0 | formatNumber}}</span>
          </template>
        </el-table-column>
         <!-- 毛利 -->
         <el-table-column
...
<script>
import {mchDailyTradePage,mchDailyTradePage_detail,mchDailyTrade_export,mchDayReportExcel} from '@/api/summary'
...
export default {
  ...
  data() {
    return {
      ...
      list: {
        current: 1,
        size: 80,
        mchNo: undefined, // 通道名称
        mchName: undefined, // 订单状态
        startTime:undefined,
        endTime:undefined,
      },
      bankOption:[{
  ...
  mounted() {
    this.getTableData()
   console.log(this.timeFormat('2024-08-19 00:00:00'),'=========')
  },
  methods: {
     //  日期
     dateChange1(val) {
